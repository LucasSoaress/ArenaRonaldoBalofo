...
public class BeringelaScript : MonoBehaviour
{
    ...
    private void Move()
    {
        float MoveVertical = Input.GetAxisRaw("P4_Vertical");
        float MoveHorizontal = Input.GetAxisRaw("P4_Horizontal");
    ...
    private void Atirar()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Joystick4Button0) && (MoveHorizontal <= -0.4f || MoveHorizontal >= 0.4f) && (MoveVertical <= -0.4f || MoveVertical >= 0.4f) && PodeAtirar == true && numeroDeBalas >= 0 && numeroDeBalas <= 5)
        {
    ...
    void OnCollisionEnter2D(Collision2D other)
    {
        ...
        if (other.gameObject.tag == "tomateMunicao" || other.gameObject.tag == "aboboraMunicao" || other.gameObject.tag == "cenouraMunicao")
        {
